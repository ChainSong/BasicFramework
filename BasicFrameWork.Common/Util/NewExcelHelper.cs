...
namespace BasicFramework.Common
{
    public class NewExcelHelper
    {
        ...
        public NewExcelHelper()
        {
        }

        public NewExcelHelper(string path)
        {
            this.path = path;
            this.CreateExcelConnection();
        }

        public NewExcelHelper(Stream st, string filePath)
        {
        ...
        private void CreateExcelConnection()
        {
            ...
            {
                if (extension == ".xls")
                    connStr = ConfigHelper.GetConnectionString("Excel03ConString");
                else if (extension == ".xlsx")
                    connStr = ConfigHelper.GetConnectionString("Excel07ConString");
                else
                    return;
            }

...
{
    public class IndexController : BaseController//  AsyncController// 
    {
        private object GetNav()
        {
            object cacheNav = CacheHelper.Get("Nav");
            if (cacheNav == null)
            {
                var menu = ApplicationConfigHelper.GetRoleMenus(((User)Session["UserInfo"]).RoleID).Where(m => m.Scenarios == 2 && m.IsChecked).Select(m => (Menu)m);

                CacheHelper.Insert("Nav", menu);
                cacheNav = menu;
            }
            return cacheNav;
        }

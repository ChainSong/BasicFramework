...
namespace System.Web.Mvc
{
    ...
    {
        private const string ScriptBasePath = "~/js/";
        ...
        private const string CssBasePath = "~/css/";
        ...
        private const string MUIjsPath = "mui.min.js";
        private const string MUIcssPath = "mui.min.css";
        ...
        public static MvcHtmlString LoadPageCssJs(this UrlHelper url, bool css = true, bool js = true)
        {
           ...
           // var area = routeDate.DataTokens["area"].ToString();
            ...
            if (css)
            {
                sb.AppendLine(Css(url, string.Format("{0}/{1}.css",  controller, action)).ToString());
            }

            if (js)
            {
                sb.AppendLine(Js(url, string.Format("{0}/{1}.js", controller, action)).ToString());
            }
        ...
        public static MvcHtmlString MuiResource(this UrlHelper url)
        {
            return Concat(url.FrameworkJs(MUIjsPath),
                url.FrameworkCss(MUIcssPath));
        }
